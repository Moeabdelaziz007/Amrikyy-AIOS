...
const NexusGoApp: React.FC<NexusGoAppProps> = ({ onOpenApp }) => {
    const { t } = useLanguage();
            ...
            <div className="flex-grow w-full max-w-2xl mx-auto space-y-4">
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-text-muted">
                        <NexusGoIcon className="w-20 h-20 mb-4 opacity-30" />
        ...
        <div className="p-4 sm:p-6 flex flex-col gap-6 h-full">
             <div className="flex-grow w-full max-w-2xl mx-auto space-y-4">
                <div className="text-center">
                    ...
                    <h2 className="text-xl font-bold font-display">Taxi & Ride Hailing</h2>
                    <p className="text-text-muted">{t('nexus_go.current_location')}</p>
                    {error && <p className="text-sm text-red-400 mt-2">{error}</p>}
                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {results.options.map(option => (
                                <div key={option.service} className="bg-black/20 p-3 rounded-lg border border-border-color flex items-center gap-3">
                                    {option.providerLogo && <img src={option.providerLogo} alt={option.service} className="w-10 h-10 object-contain"/>}
                                    <div>
                                        <p className="font-bold text-sm">{option.service}</p>
                                        <p className="text-xs text-text-muted">{option.estimatedTime} ({option.eta} ETA)</p>
                                    </div>
                                    <div className="ml-auto text-right">
                                        <p className="font-semibold text-sm">{option.estimatedCost}</p>
                                        {option.surgePricing && <span className="text-xs text-red-400">Surge Pricing</span>}
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="flex-shrink-0 border-t border-white/10 pt-4">
                <div className="flex gap-2">
                    ...
                    <button 
                        ...
                        className="h-12 px-6 font-bold rounded-lg bg-gradient-to-r from-primary-blue to-blue-500 hover:brightness-110 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {t('nexus_go.find_options')}
                    </button>
                </div>
