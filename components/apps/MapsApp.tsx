            ...
            <div className="flex-grow p-6 flex flex-col items-center justify-center gap-6 overflow-y-auto">
                ...
                <div className="w-full max-w-lg p-4 bg-black/20 border border-white/10 rounded-xl min-h-[100px] flex items-center justify-center">
                    {isLoading ? (
                        ...
                        <p className="text-sm text-left whitespace-pre-wrap">{response}</p>
                    ) : null}
                </div>
            </div>
            <div className="p-4 border-t border-white/10">
                <div className="relative">
                    <input
                        ...
                        placeholder="e.g., 'restaurants near me' or 'directions to JFK'"
                        disabled={isLoading || !location}
                        className="w-full h-12 bg-white/5 border border-white/10 rounded-full pl-5 pr-14 text-text-primary focus:ring-2 focus:ring-emerald-500 focus:outline-none transition-all duration-300"
                    />
                    <button
                        onClick={handleSend}
                        disabled={isLoading || !input || !location}
                        aria-label="Send map query"
                        className="absolute right-2 top-1/2 -translate-y-1/2 h-9 w-9 bg-emerald-500 rounded-full flex items-center justify-center hover:bg-emerald-600 transition-colors disabled:bg-gray-500"
                    >
